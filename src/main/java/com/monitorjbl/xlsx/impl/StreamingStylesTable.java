...
public class StreamingStylesTable extends POIXMLDocumentPart implements Serializable {
  ...
  private final List<StreamingFont> fonts = new ArrayList<StreamingFont>();
  private final List<StreamingCellFill> fills = new ArrayList<StreamingCellFill>();
  private final List<StreamingCellBorder> borders = new ArrayList<StreamingCellBorder>();
  ...
  public StreamingStylesTable(PackagePart part, PackageRelationship rel) throws IOException {
    super(part, rel);
  ...
  /**
   * Get the current Workbook's theme table, or null if the Workbook lacks any
   * themes.
   * <p>
   * Use {@link #ensureThemesTable()} to have a themes table created if needed
   * 
   * @return the {@code ThemesTable} themes table
   */
  ...
  public void setTheme(ThemesTable theme) {
    ...
    for (StreamingFont font : fonts) {
      font.setThemesTable(theme);
    }
    for (StreamingCellBorder border : borders) {
      border.setThemesTable(theme);
    }
  }

  /**
   * If there isn't currently a {@link ThemesTable} for the current Workbook, then
   * creates one and sets it up. After this, calls to {@link #getTheme()} won't
   * give null
   */
   ...
   */
  @SuppressWarnings("deprecation") // YK: getXYZArray() array accessors are deprecated in xmlbeans with JDK 1.5
                                   // support
    ...
    try {
      ...
      if (ctfonts != null) {
        ...
        for (CTFont font : ctfonts.getFontArray()) {
          ...
          StreamingFont f = new StreamingFont(font, idx);
          fonts.add(f);
          idx++;
        }
      }
      ...
      if (ctfills != null) {
        for (CTFill fill : ctfills.getFillArray()) {
          fills.add(new StreamingCellFill(fill));
        }
      }
      ...
      if (ctborders != null) {
        for (CTBorder border : ctborders.getBorderArray()) {
          borders.add(new StreamingCellBorder(border));
        }
      }
  ...
  public int putNumberFormat(String fmt) {
    ...
    throw new IllegalStateException("The maximum number of Data Formats was exceeded. " + "You can define up to "
        + usedNumberFormats.length + " formats in a .xlsx Workbook");
  }

  /**
   * Returns the font for the specified index
   * 
   * @param idx the font index
   * @return the {@code StreamingFont} font object, or {@code null} if no font has
   *         been registered for the required index
   */
  public StreamingFont getFontAt(int idx) {
    return fonts.get(idx);
  }

  /**
   * Records the given font in the font table. Will re-use an existing font index
   * if this font matches another, EXCEPT if forced registration is requested.
   * This allows people to create several fonts then customise them later. Note -
   * End Users probably want to call {@link XSSFFont#registerTo(StylesTable)}
   * 
   * @param font              the {@code StreamingFont} font to be added
   * @param forceRegistration {@code true} to force the registration of the
   *                          specified font even if already registered,
   *                          {@code false} otherwise
   * 
   * @return the index of the registered font
   */
  public int putFont(StreamingFont font, boolean forceRegistration) {
    int idx = -1;
  ...
  public int putFont(StreamingFont font) {
    return putFont(font, false);
  }

  public StreamingCellStyle getStyleAt(int idx) {
    int styleXfId = 0;
    ...
    return new StreamingCellStyle(idx, styleXfId, this, theme);
  }

  public int putStyle(StreamingCellStyle style) {
  ...
  public StreamingCellBorder getBorderAt(int idx) {
    return borders.get(idx);
  }

  public int putBorder(StreamingCellBorder border) {
  ...
  public StreamingCellFill getFillAt(int idx) {
    return fills.get(idx);
  }

  public List<StreamingCellBorder> getBorders() {
    return borders;
  }

  public List<StreamingCellFill> getFills() {
    return fills;
  }

  public List<StreamingFont> getFonts() {
    return fonts;
  }
  ...
  public int putFill(StreamingCellFill fill) {
  ...
  /**
   * Adds the specified cell style
   * 
   * @param cellStyleXf the {@code CTXf} cell style
   ...
   * 
   * @return the number of the cell styles
   */
   ...
   * For unit testing only!
   * 
   * @return the {@code CTStylesheet} object
   */
  ...
  public void writeTo(OutputStream out) throws IOException {
    ...
    idx = 0;
    for (StreamingCellBorder b : borders)
  ...
  private void initialize() {
    ...
    StreamingFont xssfFont = createDefaultFont();
    fonts.add(xssfFont);

    CTFill[] ctFill = createDefaultFills();
    fills.add(new StreamingCellFill(ctFill[0]));
    fills.add(new StreamingCellFill(ctFill[1]));

    CTBorder ctBorder = createDefaultBorder();
    borders.add(new StreamingCellBorder(ctBorder));
  ...
  private static StreamingFont createDefaultFont() {
    CTFont ctFont = CTFont.Factory.newInstance();
    StreamingFont xssfFont = new StreamingFont(ctFont, 0);
  ...
  public StreamingCellStyle createCellStyle() {
    if (getNumCellStyles() > MAXIMUM_STYLE_ID) {
      throw new IllegalStateException("The maximum number of Cell Styles was exceeded. " + "You can define up to "
          + MAXIMUM_STYLE_ID + " style in a .xlsx Workbook");
    }
    ...
    return new StreamingCellStyle(indexXf - 1, xfSize - 1, this, theme);
  }

  /**
   * Finds a font that matches the one with the supplied attributes
   * 
   * @param boldWeight the weight of boldness
   * @param color      the color
   * @param fontHeight the height
   * @param name       the name
   * @param italic     weather is italic or not
   * @param strikeout  weather is strikeout or not
   * @param typeOffset the type offset
   * @param underline  the underline
   * 
   * @return the {@code StreamingFont} font
   */
  public StreamingFont findFont(short boldWeight, short color, short fontHeight, String name, boolean italic,
      boolean strikeout, short typeOffset, byte underline) {
    for (StreamingFont font : fonts) {
      if ((font.getBoldweight() == boldWeight) && font.getColor() == color && font.getFontHeight() == fontHeight
          && font.getFontName().equals(name) && font.getItalic() == italic && font.getStrikeout() == strikeout
          && font.getTypeOffset() == typeOffset && font.getUnderline() == underline) {
        return font;
      }
    }
    return null;
  ...
  public static StreamingStylesTable from(StylesTable stylesTable) throws IOException {
    return new StreamingStylesTable(stylesTable.getPackagePart(), stylesTable.getPackageRelationship());
  }
}
