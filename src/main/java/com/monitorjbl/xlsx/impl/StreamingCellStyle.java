...
import com.monitorjbl.xlsx.impl.extensions.StreamingCellBorder;
import com.monitorjbl.xlsx.impl.extensions.StreamingCellBorder.BorderSide;
import com.monitorjbl.xlsx.impl.extensions.StreamingCellFill;

public class StreamingCellStyle implements CellStyle, Serializable {

  private static final long serialVersionUID = 1L;

  private int _cellXfId;
  private StreamingStylesTable _stylesSource;
  private CTXf _cellXf;
  private CTXf _cellStyleXf;
  private StreamingFont _font;
   ...
   * 
   * @param cellXfId      The main XF for the cell. Must be a valid 0-based index
   *                      into the XF table
   * @param cellStyleXfId Optional, style xf. A value of <code>-1</code> means no
   *                      xf.
   * @param stylesSource  Styles Source to work off
   * @param theme         the {@code ThemesTable} themes table object
   */
  public StreamingCellStyle(int cellXfId, int cellStyleXfId, StreamingStylesTable stylesSource, ThemesTable theme) {
      _cellXfId = cellXfId;
   ...
   * 
   * @param stylesSource the {@code StreamingStylesTable} styles table object
   */
  public StreamingCellStyle(StreamingStylesTable stylesSource) {
      _stylesSource = stylesSource;
  ...
  /**
   * Verifies that this style belongs to the supplied Workbook Styles Source. Will
   * throw an exception if it belongs to a different one. This is normally called
   * when trying to assign a style to a cell, to ensure the cell and the style are
   * from the same workbook (if they're not, it won't work)
   * 
   * @param src the {@code StreamingStylesTable} styles table object
   * @throws IllegalArgumentException if there's a workbook mis-match
   */
  public void verifyBelongsToStylesSource(StreamingStylesTable src) {
   ...
   * This XSSFCellStyle will then have all the same properties as the source, but
   * the two may be edited independently. Any stylings on this XSSFCellStyle will
   * be lost!
   *
   * The source XSSFCellStyle could be from another XSSFWorkbook if you like. This
   * allows you to copy styles from one XSSFWorkbook to another.
   */
  @Override
  public void cloneStyleFrom(CellStyle source) {
    if (source instanceof StreamingCellStyle) {
      StreamingCellStyle src = (StreamingCellStyle) source;
      ...
      if (src._stylesSource == _stylesSource) {
        ...
        try {
          ...
          _cellXf = CTXf.Factory.parse(src.getCoreXf().toString());

          // bug 56295: ensure that the fills is available and set correctly
          CTFill fill = CTFill.Factory.parse(src.getCTFill().toString());
          addFill(fill);
        ...
        setDataFormat((new StreamingDataFormat(_stylesSource)).getFormat(fmt));

        // Copy the font
        try {
          CTFont ctFont = CTFont.Factory.parse(src.getFont().getCTFont().toString());
  ...
  private void addFill(CTFill fill) {
    int idx = _stylesSource.putFill(new StreamingCellFill(fill));
   ...
   *
   * @return short - border type, default value is
   *         {@link org.apache.poi.ss.usermodel.CellStyle#BORDER_NONE}
   * @see org.apache.poi.ss.usermodel.CellStyle#BORDER_NONE
   * @see org.apache.poi.ss.usermodel.CellStyle#BORDER_THIN
   * @see org.apache.poi.ss.usermodel.CellStyle#BORDER_MEDIUM
   * @see org.apache.poi.ss.usermodel.CellStyle#BORDER_DASHED
   ...
   * Get the type of border to use for the left border of the cell
   *
   * @return border type, default value is
   *         {@link org.apache.poi.ss.usermodel.BorderStyle#NONE}
   */
   ...
   * </p>
   * 
   * @return the index of the color definition, default value is
   *         {@link org.apache.poi.ss.usermodel.IndexedColors#AUTOMATIC}
  ...
  public XSSFColor getBottomBorderXSSFColor() {
    ...
    int idx = (int) _cellXf.getBorderId();
    StreamingCellBorder border = _stylesSource.getBorderAt(idx);
  ...
  public String getDataFormatString() {
    ...
    return new StreamingDataFormat(_stylesSource).getFormat((short) idx);
  }

  /**
   * Get the background fill color.
   * <p>
   * Note - many cells are actually filled with a foreground fill, not a
   * background fill - see {@link #getFillForegroundColor()}
   * </p>
   * 
   * @return fill color, default value is
   *         {@link org.apache.poi.ss.usermodel.IndexedColors#AUTOMATIC}
   * @see org.apache.poi.ss.usermodel.IndexedColors
   */
  @Override
  ...
  public XSSFColor getFillBackgroundXSSFColor() {
    // bug 56295: handle missing applyFill attribute as "true" because Excel does as
    // well
    if (_cellXf.isSetApplyFill() && !_cellXf.getApplyFill())
      return null;

    int fillIndex = (int) _cellXf.getFillId();
    StreamingCellFill fg = _stylesSource.getFillAt(fillIndex);
   ...
   * <p>
   * Many cells are filled with this, instead of a background color
   * ({@link #getFillBackgroundColor()})
   * </p>
   * 
   * @see IndexedColors
   * @return fill color, default value is
   ...
   * 
   * @return fill pattern, default value is
   *         {@link org.apache.poi.ss.usermodel.CellStyle#NO_FILL}
   *
  ...
  public short getFillPattern() {
    ...
    int fillIndex = (int) _cellXf.getFillId();
    StreamingCellFill fill = _stylesSource.getFillAt(fillIndex);
   ...
   * Get the fill pattern
   *
   * @return the fill pattern, default value is
   *         {@link org.apache.poi.ss.usermodel.FillPatternType#NO_FILL}
   */
  ...
  /**
   * Get the index within the StylesTable (sequence within the collection of CTXf
   * elements)
   *
  ...
  /**
   * Workaround for places where we need to support more than 32767 cell styles,
   * ideally the main getIndex() and others would return int, not short, but that
   * would affect some public APIs
   *
   * @return unique index number of the underlying record this style represents,
   *         as an int (always positive)
   */
   ...
   *
   * @return the index of the color definition, default value is
   *         {@link org.apache.poi.ss.usermodel.IndexedColors#BLACK}
   * @see org.apache.poi.ss.usermodel.IndexedColors
   */
  @Override
   ...
   * <p>
   * Expressed in degrees. Values range from 0 to 180. The first letter of the
   * text is considered the center-point of the arc.
   * </p>
   * <p>
   * For 0 - 90, the value represents degrees above horizon. For 91-180 the
   * degrees below the horizon is calculated as:
   * </p>
   * <p>
   ...
   *
   * @return align the type of alignment, default value is
   *         {@link org.apache.poi.ss.usermodel.CellStyle#VERTICAL_BOTTOM}
   ...
   * Get the type of vertical alignment for the cell
   *
   * @return the type of alignment, default value is
   *         {@link org.apache.poi.ss.usermodel.VerticalAlignment#BOTTOM}
   ...
   *
   * @return a boolean value indicating if the text in a cell should be
  ...
  public void setBorderBottom(short border) {
    ...
    int idx = _stylesSource.putBorder(new StreamingCellBorder(ct, _theme));
   ...
   * </pre>
   * 
   * optionally a Foreground and background fill can be applied: <i>Note: Ensure
   * Foreground color is set prior to background</i>
   * 
   * <pre>
   * cs.setFillPattern(XSSFCellStyle.FINE_DOTS);
   ...
   * </pre>
   * 
   * It is necessary to set the fill style in order for the color to be shown in
   * the cell.
   *
  ...
  /**
   * Get a <b>copy</b> of the currently used CTFill, if none is used, return a new
   * instance.
   */
  private CTFill getCTFill() {
    ...
    if (!_cellXf.isSetApplyFill() || _cellXf.getApplyFill()) {
      int fillIndex = (int) _cellXf.getFillId();
      StreamingCellFill cf = _stylesSource.getFillAt(fillIndex);
  ...
  /**
   * Get a <b>copy</b> of the currently used CTBorder, if none is used, return a
   * new instance.
   */
  private CTBorder getCTBorder() {
    ...
    if (_cellXf.getApplyBorder()) {
      int idx = (int) _cellXf.getBorderId();
      StreamingCellBorder cf = _stylesSource.getBorderAt(idx);
   ...
   * color cell fills. For solid cell fills (no pattern), foregorund color is
   * used. For cell fills with patterns specified, then the cell fill color is
   ...
   * @param fp fill pattern (set to
   *           {@link org.apache.poi.ss.usermodel.CellStyle#SOLID_FOREGROUND} to
  ...
  public void setFillPattern(short fp) {
    ...
    if (fp == NO_FILL && ptrn.isSetPatternType())
  ...
  /**
   * This element is used to specify cell fill information for pattern and solid
   * color cell fills. For solid cell fills (no pattern), foreground color is used
   * is used. For cell fills with patterns specified, then the cell fill color is
   * specified by the background color element.
   *
   ...
   * <p>
   * Setting this flag to <code>true</code> make all content visible whithin a
   * cell by displaying it on multiple lines
   * </p>
   *
   * @param wrapped a boolean value indicating if the text in a cell should be
   *                line-wrapped within the cell.
   */
  @Override
  ...
  /**
   * Returns a hash code value for the object. The hash is derived from the
   * underlying CTXf bean.
   *
  ...
  public boolean equals(Object o) {
    if (o == null || !(o instanceof StreamingCellStyle))
      return false;

    StreamingCellStyle cf = (StreamingCellStyle) o;
    return _cellXf.toString().equals(cf.getCoreXf().toString());
  }

  /**
   * Make a copy of this style. The underlying CTXf bean is cloned, the references
   * to fills and borders remain.
   *
  ...
  public Object clone() {
    ...
    return new StreamingCellStyle(indexXf - 1, xfSize - 1, _stylesSource, _theme);
  }
}
